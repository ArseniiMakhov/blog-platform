...
export const ArticlePage = () => {
  ...
  const { loading } = rootReducer

  const tags = item.tagList?.length ? (
    item.tagList.slice(0, 15).map((el, index) => {
      if (!el.trim().length) {
        return (
  ...
  if (item) {
    return (
      <Card className={`${classes.article} ${classes.page}`} bordered={false}>
        <Meta
          title={
              ...
              <div className={classes['header-left']}>
                <div className={classes['left-box']}>
                  ...
                  <p className={classes['header-likes']}>
                    ...
                    <span className={classes['likes-counter']}>{item.favoritesCount}</span>
                  </p>
                </div>
                {tags}
              </div>
          ...
          description={
            <div className={classes['article-description']}>
              ...
              <Markdown className={classes['description-body']}>{item.body}</Markdown>
            </div>
          }
        />
      </Card>
